...
class StandardizedCustomContent {
	...
	* Create the new Custom Fields meta box
	...
	{
		//$current_post_data = get_post_data($post_id);
		...
		{			
			...
			foreach ( $custom_fields as $field ) {
				if ( isset( $_POST[ self::$prefix . $field['name'] ] ) )
				{
					...
					update_post_meta( $post_id, 'trend_create_date', $post->post_date );
				}
				// if not set, then it's an unchecked checkbox, so blank out the value.
				else 
				{
					update_post_meta( $post_id, $field[ 'name' ], '' );
					update_post_meta( $post_id, $field[ 'trend_create_date' ], '' );
				}
			}
	...
	static function update_trend_meta( $new_status, $old_status, $post )
	{ 
		// when making quick edit posts, update the trend_create_date so that the ranking doesn't get affected.
	    update_post_meta( $post->ID, 'trend_create_date', $post->post_date );
	}
